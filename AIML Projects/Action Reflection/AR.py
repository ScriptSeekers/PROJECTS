...
# Try different ways to import pose
try:
    ...
    try:
        ...
        import mediapipe.tasks.vision as tasks
        # If it gets here, it's a very new version using tasks API
        # But let's try to stick to solutions if possible
        raise ImportError("Could not find mediapipe solutions. Please ensure mediapipe is installed correctly.")
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        # Use original landmarks, not mirrored
        ...
        for t_idx, trail_frame in enumerate(trail_points[:-1]):
            ...
            for x, y in trail_frame:
                if 0 <= x < width and 0 <= y < height:
                    cv2.circle(glow_layer, (x, y), 6, trail_color, -1)
    ...
    cv2.setWindowProperty("CLONE TRACKER", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
